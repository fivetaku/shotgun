...
- **wake 모드**: 세션이 유휴 상태라 몇 초 안에 아무도 플래그를 안 먹으면, 데몬이 터미널에 `/shotgun:bang` + 엔터를 대신 타이핑해 세션을 강제로 깨웁니다 (tmux면 정확한 페인, 아니면 전면 창). `/bang`은 플러그인의 정식 커맨드라서 정해진 워크플로우 — **사과 먼저** → 강도/콤보 확인 → 재검토 → 재실행 — 가 그대로 돕니다. 당신은 아무 입력도 할 필요 없습니다. macOS에선 손쉬운 사용(Accessibility) 권한 필요, 끄려면 `~/.claude/shotgun/config.json`에서 `"wake": false`.
