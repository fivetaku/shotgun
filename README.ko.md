...
- 세션 수명 데몬이 ffmpeg로 마이크를 읽어 21ms 볼륨 블록을 롤링 배경 소음과 비교합니다. 캘리브레이션된 임계값(기본 5000)을 넘는 급격한 스파이크 = 샷건.
...
- **자가 감시 (v0.4)**: 세션이 시작될 때 스스로 플래그 파일 감시를 무장합니다. 쾅 치면 유휴 세션이 **그 이벤트로 직접 깨어나** 사과부터 시작합니다 — 키 입력 주입이 아니라서 포커스·창 위치·한글 IME·앱 종류가 전혀 상관없습니다.
- **wake 폴백**: 감시가 없는 세션(첫 턴 이전 등)을 위해, 데몬이 마지막 활동 세션의 터미널에 `/shotgun:bang`을 대신 타이핑하는 폴백이 남아 있습니다 (tmux면 정확한 페인). 감시가 먼저 플래그를 소비하면 자동 취소됩니다. macOS에선 손쉬운 사용(Accessibility) 권한 필요, 끄려면 `~/.claude/shotgun/config.json`에서 `"wake": false`.
