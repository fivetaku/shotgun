...
- **Self-watching sessions (v0.4)**: each session arms a background watch on the flag file at startup. Slam, and an idle session wakes ITSELF on the event and apologizes first — no keystroke injection, so focus, window position, IME, and app quirks don't matter.
- **Wake fallback**: for sessions without a watch (before their first turn), the daemon types `/shotgun:bang` into the most recently active session's terminal (exact tmux pane when available). It cancels itself when a watch consumes the flag first. Needs Accessibility permission on macOS; disable with `"wake": false` in `~/.claude/shotgun/config.json`.
